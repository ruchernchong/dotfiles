...
case "$INSTALL_PROFILE" in
    minimal|developer|full)
        ...
        ;;
    *)
        echo "Error: Invalid profile '$INSTALL_PROFILE'. Valid options: minimal, developer, full" >&2
        exit 1
        ;;
esac
...
if [[ "$INTERACTIVE_MODE" == "true" ]]; then
    ...
    # Non-interactive mode: set sensible defaults
    BACKUP_EXISTING=false
    BACKUP_DIR=""
    SKIP_EXISTING=false
    REPLACE_CRONTAB=true
    SELECTED_NODE_MANAGER="fnm"
    SELECTED_PYTHON_VERSION="3.13"
    INSTALL_DATABASES=true
...
# Write state file for shell scripts to source
